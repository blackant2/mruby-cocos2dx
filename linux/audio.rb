...
LOCAL_SRC_FILES = %w(SimpleAudioEngineFMOD.cpp FmodAudioPlayer.cpp )
...
LOCAL_SRC_FILES.each do |f|
   ...
   sh="#{CC} -DCC_ENABLE_CHIPMUNK_INTEGRATION=1 "
		...
		sh<<" -I#{COCOS_ROOT}/cocos/audio/../../external/linux-specific/fmod/include/64-bit   "
		
		sh<<" -o objs/#{LIB_NAME}/#{f}.o  "
		sh<<" -c #{COCOS_ROOT}/cocos/audio/linux/#{f}"
