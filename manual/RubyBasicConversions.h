...
*/
extern struct mrb_data_type ruby_cocos2dx_Ref_type;
template <class T>
...
{
    //CCLOG(">>>>rubyval_to_object start");
    if (arg.tt != mrb_vtype::MRB_TT_DATA) {
        // TODO: support ruby object
  //      CCLOG("%s is not MRB_TT_DATA start!!",type);
        return false;
    }
    ...
    if (! match) {
//        CCLOG("%s is not  match!!",mrb_class_name(mrb, rclass));
        return false;
    }
    ...
    *ret = static_cast<T*>(mrb_get_datatype(mrb, arg, data_type));
    ...
    if (std::is_base_of<cocos2d::Ref, T>::value) {
      ...
      //      CCLOG("new _scriptObject point @0x%x",(unsigned int)(void *)ref->_scriptObject);
        }else{
        //    CCLOG("_scriptObject point @0x%x",(unsigned int)(void *)ref->_scriptObject);
        }
    }
    else{
       // CCLOG("%s is not base of cocos2d::Ref",type);
    }   
    return true;
}
...
{
    //CCLOG("<<<<object_to_rubyval start");
    if (nullptr == ret) {
      //    CCLOG("object_to_rubyval is null");
        return mrb_nil_value();
    }
    ...
    if (std::is_base_of<cocos2d::Ref, T>::value) {
        ...
        if (nullptr != ref->_scriptObject) {
            _ScriptObject* sobj = (_ScriptObject *)ref->_scriptObject;
        //    CCLOG("get mrb_val  from _ScriptObject");
            return sobj->mrb_val;
        }
    }
    else{
        //CCLOG("%s not base of cocos2d::Ref!!!",type);
    }
    
    const char* ruby_type = getRubyTypeName(ret, type);
    if (nullptr == rclass) {
        //CCLOG("get rclass  from ruby_type %s",ruby_type);
...
#if CC_ENABLE_SCRIPT_BINDING_MEMORY_CONTROL
#if COCOS2D_DEBUG >= 1
   // CCLOG("CC::Ref object to Ruby object (cc::Ref) %s@%x=>%s",type, (unsigned int)(void *)ret ,ruby_type);
#endif
        ref->retain();
#endif
    }
